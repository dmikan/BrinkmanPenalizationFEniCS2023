...
def time_stepping(V, A, L, u0, u1, p1, w1, bcu, bcp, ufile, pfile, wfile):
# Time-stepping
    t = 0
    time_step = 0
    ...
    while t < FINAL_TIME + DOLFIN_EPS:   
        ...
        curl_u1_proj = dolfin.project(curl_u1)
        w1.assign(curl_u1_proj)
...
ufile_ref = dolfin.File("results/results_ref/velocity.pvd")
pfile_ref = dolfin.File("results/results_ref/pressure.pvd")
wfile_ref = dolfin.File("results/results_ref/vorticity.pvd")
...
time_stepping(V_ref, A_ref, L_ref, u0_ref, u1_ref, p1_ref, w1_ref, bcu_ref, bcp_ref, ufile_ref, pfile_ref, wfile_ref)

# Output filename
output_filename = "results/data_{}.txt".format(FINAL_TIME)
...
for eta_brinkman in eta_values:
    ...
    # Create files for storing solution
    ufile_penal = dolfin.File("results/results_{}/velocity.pvd".format(eta_brinkman))
    pfile_penal = dolfin.File("results/results_{}/pressure.pvd".format(eta_brinkman))
    wfile_penal = dolfin.File("results/results_{}/vorticity.pvd".format(eta_brinkman))
    ...
    time_stepping(V_penal, A_penal, L_penal, u0_penal, u1_penal, p1_penal, w1_penal, bcu_penal, bcp_penal, ufile_penal, pfile_penal, wfile_penal)
