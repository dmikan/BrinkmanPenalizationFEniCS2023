...
mesh_ref = Mesh('../mesh2/mesh-square.xml')
...
mesh_penal = Mesh('../mesh2/mesh-square-penal.xml')
...
bcp_outflow_ref = dolfin.DirichletBC(Q_ref, dolfin.Constant(0), "x[0] > 2.0 - DOLFIN_EPS")
...
# Outflow boundary condition for pressure
bcp_outflow_penal = dolfin.DirichletBC(Q_penal, dolfin.Constant(0), "x[0] > 2.0 - DOLFIN_EPS")
...
def time_stepping(A, L, u0, u1, p1, w1, bcu, bcp):
# Time-stepping
    t = dt
    time_step = 1
    u1_arr = []
...
for u1_ref_sample in u1_arr_ref:
    diff_square = (u1_ref_sample)**2*dx
    error_L2_omega = np.sqrt(np.abs(dolfin.assemble(diff_square))) #norm_L2_omega_f (ref)
    error_L2_omega_arr.append(error_L2_omega) #norm_L2_omega_f_arr (ref)

# Calculate L2 norm in time in [0,T] X Ω_s 
error_L2_omega_arr = [error ** 2 for error in error_L2_omega_arr]
error_L2_time_omega_f_ref = np.sqrt(dt*np.sum(error_L2_omega_arr))    
...
with open(output_filename, 'w') as out_txt:
    ...
    out_txt.write('STEP TIME = {}\n'.format(dt))
    out_txt.write('FINAL TIME = {}\n'.format(FINAL_TIME))
    out_txt.write('REYNOLDS NUMBER = {}\n'.format(Re))
    out_txt.write('L2 NORM IN TIME AND OMEGA F OF THE REFERENCE VELOCITY = %.2e\n' % error_L2_time_omega_f_ref)
    out_txt.write('--------------------------------------------------------\n\n')
    ...
    out_txt.write('L2NTOS = L2 NORM IN TIME AND OMEGA_S\n')
    out_txt.write('L2ETOF = L2 ERROR IN TIME AND OMEGA_F\n')
    out_txt.write('L2ETOF-REL = RELATIVE L2 ERROR IN TIME AND OMEGA_F \n')
    out_txt.write('--------------------------------------------------------\n\n')
    out_txt.write('ETA\t\t')
    out_txt.write('L2NTOS\t\t')
    out_txt.write('L2ETOF\t\t')
    out_txt.write('L2ETOF-REL\n')
...
for eta_brinkman in eta_values:
    ...
    # Create arrays
    u1_arr_penal_proj = []
    error_L2_omega_arr = [] # error_L2_omega_f_arr, norm_L2_omega_s_arr
    ...
    u1_arr_penal = time_stepping(A_penal, L_penal, u0_penal, u1_penal, p1_penal, w1_penal, bcu_penal, bcp_penal)
    ...
    for index, u1_penal_proj in enumerate(u1_arr_penal_proj):
        ...
        error_L2_omega = np.sqrt(np.abs(dolfin.assemble(diff_square))) #error_L2_omega_f
        error_L2_omega_arr.append(error_L2_omega) #error_L2_omega_f_arr
        
    # Calculate L2 error in time in [0,T] X Ω_f
    error_L2_omega_arr = [error ** 2 for error in error_L2_omega_arr]
    error_L2_time_omega_f = np.sqrt(dt*np.sum(error_L2_omega_arr))
    ...
    for u1_penal_sample in u1_arr_penal:
        ...
        error_L2_omega = np.sqrt(np.abs(dolfin.assemble(diff_square))) #norm_L2_omega_s
        error_L2_omega_arr.append(error_L2_omega) #norm_L2_omega_s_arr

    # Calculate L2 norm in time in [0,T] X Ω_s 
    error_L2_omega_arr = [error ** 2 for error in error_L2_omega_arr]
    error_L2_time_omega_s = np.sqrt(dt*np.sum(error_L2_omega_arr))

    # Relative error 
    error_L2_time_omega_relative = error_L2_time_omega_f/error_L2_time_omega_f_ref
    
    # Save the values in error_data.txt
    with open(output_filename, 'a') as out_txt:
        out_txt.write('%.0e\t' % eta_brinkman)
        out_txt.write('%.2e\t' % error_L2_time_omega_s)
        out_txt.write('%.2e\t' % error_L2_time_omega_f)
        out_txt.write('%.2e\t' % error_L2_time_omega_relative)
        out_txt.write('\n')
    count += 1
