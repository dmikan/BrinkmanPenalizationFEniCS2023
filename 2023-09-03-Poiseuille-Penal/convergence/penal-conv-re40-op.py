...
for eta_brinkman in eta_values:
    ...
    for index, u1_penal_proj in enumerate(u1_arr_penal_proj):
        ...
        u1_penal_proj = np.sqrt(np.abs(dolfin.assemble(diff_square))) #error_L2_omega_f 
        #error_L2_omega_f_arr.append(error_L2_omega_f)
        
    # Calculate L2 error in time in [0,T] X Ω_f
    u1_arr_penal_proj = [error ** 2 for error in u1_arr_penal_proj] #error_L2_omega_f_arr_square, error_L2_omega_f_arr
    error_L2_time_omega_f = np.sqrt(dt_samples*np.sum(u1_arr_penal_proj))

    # Calculate L2 norm in Ω_s
    for u1_penal_sample in u1_arr_penal:
        diff_square = (chi*u1_penal_sample)**2*dx
        u1_penal_sample = np.sqrt(np.abs(dolfin.assemble(diff_square))) #norm_L2_omega_s 
        #norm_L2_omega_s_arr.append(u1_penal_sample) 

    # Calculate L2 norm in time in [0,T] X Ω_s 
    u1_arr_penal = [error ** 2 for error in u1_arr_penal] #norm_L2_omega_s_arr_square, norm_L2_omega_s_arr
    norm_L2_time_omega_s = np.sqrt(dt_samples*np.sum(u1_arr_penal))
