...
dt = 0.001
T = 0.030 
time_mult = 1/dt # take errors at integer values from 1 to T
...
for eta_brinkman in eta_values:
    ...
    # Print message
    print(f'\nComputing {count} cycle of {len(eta_values)} for eta = {eta_brinkman}...\n')
    ...
    _, u1_arr_penal, p1_arr_penal = time_stepping(dt, T, A_penal, L_penal, u0_penal, u1_penal, p1_penal, ufile_penal, pfile_penal, bcu_penal, bcp_penal)

    #project penalized solution on reference domain    
    for p1_penal_sample in p1_arr_penal:
        p1_penal_proj = dolfin.interpolate(p1_penal_sample, Q_ref)
        p1_arr_penal_proj.append(p1_penal_proj)
